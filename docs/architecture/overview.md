...
```text
React UI → Tauri Commands / Events → Rust Application Core → PTY / OpenSSH / Telnet / Serial / Storage
```
...
本地终端、OpenSSH、Telnet 与串口都遵循 `Connecting → Connected → Closed` 状态边界。创建命令先为会话 ID 建立连接占位，再把阻塞工作交给后台线程；关闭标签会设置该次连接专属的取消标记。读取线程结束时只能清理持有同一标记的当前实例，旧任务不能误删后续重建的会话。
...
快照带有固定版本和明确的行数、字符数上限。连接元数据只保留本地 Shell、Telnet/SSH 主机/端口或串口名称/波特率，不包含用户名、密码、终端输入或其他凭据。当前提供活动会话和可见分区的上下文读取能力，但本阶段不包含模型、供应商 SDK、提示词编排或 Agent 执行。
...
SSH 使用 Windows 系统 OpenSSH，并沿用本地 PTY 的输入、输出、尺寸同步和会话清理路径。连接默认使用 `StrictHostKeyChecking=accept-new`：首次连接记录主机密钥，已记录的密钥发生变化时拒绝连接。虚拟机重装等明确场景可在连接对话框中勾选一次性更新主机密钥；应用会先用 `ssh-keygen -R` 清理对应主机/端口的旧记录，再让 OpenSSH 按同一校验策略记录新密钥。该选项不会写入保存的连接资料，也不会关闭主机密钥校验。
