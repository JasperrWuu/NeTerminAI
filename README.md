...
- 字体、字号、字重、行距、光标、滚动缓冲与 ANSI 配色
- 独立设置页面与左侧设置导航；终端设置和快捷键设置互不受会话分区影响
- 可命名、折叠、启用/停用和切换的终端突显集，每套可包含多条文本或正则颜色规则
...
- RDP，在工作区内按系统已注册版本探测并宿主 Windows 官方远程桌面控件，不再启动外部 `mstsc.exe`
- 会话分区、保存、重命名与折叠；单击编辑，双击连接
- 多会话标签与独立终端状态，支持拖拽形成可递归嵌套的左右或上下分屏
- 可自定义工作台快捷键，支持可见字符终端同步输入、取消同步、会话顺序切换、`Ctrl++` 均衡分区与 `Ctrl+-` 合并分区
