...
- 字体、字号、字重、行距、光标、滚动缓冲和 ANSI 配色自定义
...
如果公司网络无法访问 crates.io，可在本机的 Cargo 配置中使用公司的 Sparse 镜像。镜像地址必须以 `/` 结尾：
...
registry = "sparse+https://your-company.example/crates.io-index/"
```

建议把镜像和证书策略保留在开发电脑的用户级配置中，不要提交公司的内部地址，也不要通过关闭证书或吊销检查来绕过安全验证。
...
- RDP 密码由 Windows 远程桌面处理，不写入生成的连接文件。
