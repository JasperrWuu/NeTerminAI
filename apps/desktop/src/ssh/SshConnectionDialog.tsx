    ...
    <div className="dialog-scrim" role="presentation" onPointerDown={onCancel}>
      <form aria-labelledby="ssh-dialog-title" className="connection-dialog" onPointerDown={(event) => event.stopPropagation()} onSubmit={(event) => {
        ...
        <div className="connection-form-grid">
          ...
          <label className="form-field form-field-wide"><span>账号</span><input autoComplete="username" onChange={(event) => setField("username", event.target.value)} placeholder={connection.authentication === "config" ? "可选；留空时使用 SSH 配置" : "例如 root 或 administrator"} required={connection.authentication === "password"} value={connection.username} /></label>
          ...
          {connection.authentication === "key" && (
            <label className="form-field form-field-wide"><span>私钥文件</span><input onChange={(event) => setField("identityFile", event.target.value)} placeholder="例如 C:\Users\you\.ssh\id_ed25519" required value={connection.identityFile} /></label>
          )}
          <div className="dialog-note form-field-wide"><strong>安全认证</strong><span>{authenticationNote(connection.authentication)}</span></div>
        </div>
...
function authenticationNote(authentication: SshConnection["authentication"]) {
